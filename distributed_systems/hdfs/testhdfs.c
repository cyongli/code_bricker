...
int main(int argc, char **argv) {
    ...
    int result = 0;
    {
        ...
        fprintf(stderr, "hdfsGetCapacity: %lld\n", hdfsGetCapacity(fs));
        fprintf(stderr, "hdfsGetUsed: %lld\n", hdfsGetUsed(fs));
...

    {
      ...
      // CHECK size
      hdfsFileInfo *finfo = hdfsGetPathInfo(fs, writePath);
      fprintf(stderr, "fileinfo->mSize: == total %s\n", ((result = (finfo->mSize == strlen("Hello, Worlld") + 1)) ? "Success!" : "Failed!"));
      totalResult += (result ? 0 : 1);
      ...
      fprintf(stderr, "read == Hello, Worlld %s\n", (result = (strcmp(rdbuffer, "Hello, Worlld") == 0)) ? "Success!" : "Failed!");
...

    {
      ...
      fs = hdfsConnectAsUser("127.0.0.1", 9000, tuser);
      if(!fs) {
        ...
        const char* buffer = "Hello, Worlld!";
        tSize num_written_bytes = hdfsWrite(fs, writeFile, (void*)buffer, strlen(buffer)+1);
        fprintf(stderr, "Wrote %d bytes\n", num_written_bytes);
